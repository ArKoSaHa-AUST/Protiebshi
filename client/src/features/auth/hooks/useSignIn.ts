...
import {
  MOCK_SIGN_IN_EMAIL,
  MOCK_SIGN_IN_PASSWORD,
  MOCK_SIGN_IN_TOKEN,
  isMockSignInCredential,
} from '../utils/mockAuth';
...
export const useSignIn = (): UseSignInResult => {
  ...
  const onSubmit = useCallback(
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      if (!isMockSignInCredential(values.email, values.password)) {
        submitFailure(`Use ${MOCK_SIGN_IN_EMAIL} / ${MOCK_SIGN_IN_PASSWORD} to sign in.`);
        return;
      }

      submitSuccess(values.email, MOCK_SIGN_IN_TOKEN, 'user');
