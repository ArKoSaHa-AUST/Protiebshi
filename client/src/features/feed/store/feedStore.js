...
const getInitialPosts = () => {
  if (typeof window !== 'undefined') {
    const stored = window.localStorage.getItem('protibeshi_feed_posts');
    if (stored) {
      try {
    ...
    window.localStorage.setItem('protibeshi_feed_posts', JSON.stringify(mockPosts));
  }
  return mockPosts;
};
...
export const useFeedStore = create((set, get) => ({
  ...
  addPost: (newPost) => {
    ...
    set((state) => {
      ...
      if (typeof window !== 'undefined') {
        window.localStorage.setItem('protibeshi_feed_posts', JSON.stringify(updatedPosts));
      }
      return {
        posts: updatedPosts,
        showComposer: false,
